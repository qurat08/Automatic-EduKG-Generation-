...
class DBpediaSpotlight:
            ...
            if with_doc_sim:
                ...
                for node in concepts:
                    if node["type"] == "annotation" or node[
                        ...
                        #convert array type "np.array" to string type and store into node
                        node["initial_embedding"]= ','.join(str(i) for i in ann_embeddings)
    ...
    def _calculate_similiraty_weight(self, concept, doc_embeddings):
        ...
        try:
            ...
            concept["initial_embedding"]= ','.join(str(i) for i in concept_embeddings)
